...
โปรเจกต์นี้คือเว็บแอปพลิเคชันสำหรับบริหารจัดการการเสนอสินเชื่อ โดยเน้นการใช้งานที่ง่าย รวดเร็ว และรองรับการทำงานแยกส่วนระหว่าง **พนักงานขาย (Sale)** และ **ผู้อนุมัติ (Approver)**
...
ตาราง `loans` และ `loan_attachments` ต้องมี policy ให้สิทธิ์ anon ถึงจะทำงานครบ (อัปเดตสถานะ, แก้ไขเคส, ลบไฟล์แนบ)

ใน **Supabase Dashboard** → **SQL Editor** รันคำสั่งนี้:

```sql
-- อนุญาตให้ anon อัปเดตแถวใน loans (สำหรับปุ่มอนุมัติ/ปฏิเสธ และหน้าแก้ไขเคส)
...
-- อนุญาตให้ anon ลบแถวใน loan_attachments (สำหรับปุ่มลบไฟล์แนบในหน้าแก้ไขเคส)
CREATE POLICY "Enable delete for anonymous users"
ON public.loan_attachments
FOR DELETE
TO anon
USING (true);
```

จากนั้นไปที่ **Table Editor** → ตาราง **loans** / **loan_attachments** → **Policies** จะเห็น policy ที่เพิ่มมา

หมายเหตุ: ถ้าเปิด RLS ไว้ ต้องมี policy สำหรับ **SELECT** และ **INSERT** บน `loans` และ `loan_attachments` ด้วย (ให้ anon ใช้ได้ตามที่แอปต้องการ)